...
Ziel der Anwendung soll es sein, Machine Learning Daten, die in textueller Form (csv oder ähnliches) oft schwer zu deuten sind, fassbarer und besser deutbar zu machen. Entsprechend bot sich {vr} als Möglichkeit der räumlichen Darstellung dieser Daten an. Entsprechend soll die Anwendung in diesem Bereich als unterstützendes Werkzeug zum Einsatz kommen.
...
Dieses Verhalten tritt mehreren Tests nach zu Urteilen lediglich dann auf, wenn die Steuerung des Dialogs mittels VR-Hardware, also dem Wand, stattfindet. 
